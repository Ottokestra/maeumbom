...
class WhisperSTT:
    ...
    def _load_model(self):
        ...
        try:
            ...
            # sys.modules에서 로컬 faster_whisper 모듈 제거 (있다면)
            modules_to_remove = [key for key in sys.modules.keys() if key.startswith('faster_whisper')]
            for module_key in modules_to_remove:
                del sys.modules[module_key]
            
            # sys.path를 조작하여 site-packages의 faster_whisper만 import
            original_path = sys.path.copy()
            
            # 프로젝트 루트 경로 모두 제거
            backend_root = str(Path(__file__).parent.parent.parent.parent)
            sys.path = [p for p in sys.path if not p.startswith(backend_root) and p != '']
            
            try:
                # 외부 라이브러리 import (site-packages에서)
                import faster_whisper
                WhisperModel = faster_whisper.WhisperModel
                
                print(f"[디버그] faster_whisper 모듈 위치: {faster_whisper.__file__}")
            finally:
                # 원래 경로 복원
